...
let vars_of_process_defs lbl defs block =
  List.fold_left block ~init:[] ~f:(fun acc instr ->
      ...
      | Phi (dst, Val (typ), _phis) ->
        update_defs dst lbl defs;
        (dst, Val typ) :: acc
      | Phi (dst, Ptx (typ), _phis) ->
        update_defs dst lbl defs;
        (dst, Ptx typ) :: acc
      | Phi (dst, Fun (typ), _phis) ->
        update_defs dst lbl defs;
        (dst, Fun typ) :: acc
      | Alloc (dst, typ, _) ->
        update_defs dst lbl defs;
...
let update_instrs stack counters block =
...
let rename_vars func_args func_name phi_map blocks cfg_succ dt =
  ...
  let rec rename block_name =
    ...
    let block' = update_instrs stack counters block in
  ...
  let add_copies dst typ args =
    List.iter args ~f:(fun (lbl,arg) ->
        let instr = 
          ...
          | Ptx typ -> Ptrcpy (dst, typ, arg)
          | Fun typ -> Anon (dst, Fun typ, Some [arg], [])
        in
        let new_instrs = 
